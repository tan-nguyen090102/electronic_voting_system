...
from backend.database.database_functions import (
...
def test_select_all_stored_proc(create_test_db_connection):
    try:
        info_result_set = execute_stored_proc(
            create_test_db_connection, "select_all_from_table", ("info",)
        )
...
def test_delete_from_table(create_test_db_connection):
    ...
    try:
        ...
        info_delete_result_set = execute_stored_proc(
            create_test_db_connection,
            "select_all_from_table_with_where",
            (
                "info",
...
def test_insert_into_table_stored_proc(create_test_db_connection):
    ...
    try:
        ...
        info_insert_result_set = execute_stored_proc(
            create_test_db_connection,
            "select_all_from_table_with_where",
            (
                "info",
...
def test_update_table(create_test_db_connection):
    try:
        ...
        info_update_result_set = execute_stored_proc(
            create_test_db_connection,
            "select_all_from_table_with_where",
            (
                "info",
